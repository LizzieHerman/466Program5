...
if __name__ == '__main__':
    ...
    router_a_rt_tbl_D = {1: {0: 1,   1: '~'},
                         2: {0: '~', 1: '~'}}  # packet to host 1 through interface 0 for cost 1
    ...
    router_b_rt_tbl_D = {1: {0: '~', 1: '~'},
                         2: {0: '~', 1: 3}}  # packet to host 2 through interface 1 for cost 3
    ...
    # give the network sufficient time to send all route updates
    sleep(2)
    # create some send events
    for i in range(5):
        priority = i % 2
        print(priority)
        client.udt_send(priority, 1, 2, 'data', 'Sample client data %d' % i)
