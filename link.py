        ...
        for (node_a, node_a_intf, node_b, node_b_intf) in [
            ...
            try:
                ...
                if intf_a.next_avail_time <= time.time():
                    ...
                    if pkt_S[0] == '#':
                        mpls = network.MPLS_Frame.from_byte_S(pkt_S)
                        p = mpls.pkt
                    else:
                    ...
                    a = max(intf_a.in_queue.qsize(), intf_a.out_queue.qsize(), intf_b.in_queue.qsize(),
                            intf_b.out_queue.qsize())
                    b = max(intf_a.num_prior_in_zero, intf_a.num_prior_out_zero, intf_b.num_prior_in_zero,
                            intf_b.num_prior_out_zero)
                    c = max(intf_a.num_prior_in_one, intf_a.num_prior_out_one, intf_b.num_prior_in_one,
                            intf_b.num_prior_out_one)
