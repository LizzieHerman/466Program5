...
class NetworkPacket:
    ...
    def from_byte_S(self, byte_S):
        ...
        dst_addr = int(
            byte_S[NetworkPacket.prior_S_length: NetworkPacket.prior_S_length + NetworkPacket.dst_addr_S_length])
        prot_S = byte_S[
                 NetworkPacket.prior_S_length + NetworkPacket.dst_addr_S_length: NetworkPacket.prior_S_length + NetworkPacket.dst_addr_S_length + NetworkPacket.prot_S_length]
        if prot_S == '1':
            prot_S = 'data'
        elif prot_S == '2':
            prot_S = 'control'
...
class Host:
    ...
    def udt_receive(self):
        ...
        if pkt_S is not None:
            ...
            print('%s: received packet "%s" with priority %d' % (self, pkt_S, p.priority))
...
class Router:
    ...
    def send_routes(self, i):
        priority = 1  # i don't know what the priority should be
